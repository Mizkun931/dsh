...
Reference for the Electron desktop launcher over the shipped Web profile. The package displays a frameless DeepSeek splash window first, compiles the workspace artifacts in the background with splash progress updates, starts the built `@deepseek-ai/dsh` CLI with `web --host 127.0.0.1 --port 0`, waits for the `dsh web:` URL line, then opens that loopback URL in a sandboxed Electron window.
