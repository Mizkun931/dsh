...
Splash progress now dwells at 96% during the client boot and jumps to 100% at the switch; the fill transition smooths the jump. Build output markers still top out at 76% and server start at 92%, so the bar keeps moving forward during the longest phases.
