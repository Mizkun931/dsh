...
splash 进度在客户端启动期间停在 96%，切换时跳至 100%；fill 的过渡平滑了这一跳。构建输出标记细分为七个档位（14/24/36/46/58/70/80），静默阶段——tsc、tsdown、vite、无输出的服务端启动与客户端启动——按每 350ms +1% 估算前进，封顶在下一个真实档位之下：进度条在每个静默段持续前进，真实事件到达时立即对齐。
