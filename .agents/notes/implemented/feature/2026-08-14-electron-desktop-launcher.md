...
The desktop package owns a full-screen frameless splash window in `apps/desktop/assets/splash.html`. The splash displays only the screenshot-derived blue DeepSeek icon and wordmark, a liquid-glass `Harness` badge, and a progress bar. The bar width is driven by main-process progress events parsed from the launch-time build and Web-host readiness phases, so the foreground startup animation stays aligned with the background work. The Web shell loading page in `packages/client/web/src/AppRoot.tsx` uses the same logo-led loading language for the plugin-loading interval after the main window loads.
